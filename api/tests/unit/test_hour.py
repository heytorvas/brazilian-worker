...
class TestHourBaseModel:
    def test_create_valid_hour_base(self):
        ...
        assert hour.daily_hours is None

        hour = HourBase(raw="5000", weekly_hours="40", daily_hours="8")
        assert hour.raw == 5000
        assert hour.weekly_hours == 40
        assert hour.daily_hours == 8
    ...
    def test_create_invalid_hour_base(self):
        ...
        with pytest.raises(ValueError):
            HourBase(raw=5000, weekly_hours=40, daily_hours="opa")


class TestHourModel:
    def test_check_without_daily_hours_should_return_none(self):
        base = HourBase(raw=5000, weekly_hours=40)
        hour = calculate_hour(base)
        assert base.daily_hours is None
