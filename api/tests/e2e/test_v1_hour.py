...
class TestIntegrationHour:
    ...
    def test_calculate_salary_hour_without_daily_hour(self):
        ...
        assert response["daily_hour"] is None
        assert response["hour_value"] == 15
        assert response["day_value"] is None
        assert response["extra_hour_value"] == 22.5

    def test_calculate_salary_hour_with_daily_hour(self):
        body = {"raw": 3000, "weekly_hours": 40, "daily_hour": 8}
        response = json.loads(
            client.post(self.URL, headers=self.HEADERS, json=body).text
        )
