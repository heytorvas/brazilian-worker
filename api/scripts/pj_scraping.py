...
def get_html_body():
    r = requests.get('https://www.contabilizei.com.br/contabilidade-online/tabela-simples-nacional-completa/').text
    soup = BeautifulSoup(r, 'html.parser')
    body = soup.find('body')
    return body.find_all('figure', {'class': 'wp-block-table'})

def get_all_attaches():
    tables = get_html_body()
    attaches = ['I', 'II', 'III', 'IV', 'V']
    response = {}
    for j in range(5):
        ...
        response.update({attaches[j]: aux})
    return response